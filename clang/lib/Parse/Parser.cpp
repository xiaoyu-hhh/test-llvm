...
Parser::DeclGroupPtrTy Parser::ParseDeclOrFunctionDefInternal(
  ...
  //IClang begin
  // auto Begin = DeclSpecAttrs.Range.getBegin();
  // auto End   = DeclSpecAttrs.Range.getEnd();
  // const auto &SM = PP.getSourceManager();
  // if (Attrs.Range.isValid()) {
  //   if (Begin.isInvalid() ||
  //       SM.isBeforeInTranslationUnit(Attrs.Range.getBegin(), Begin))
  //     Begin = Attrs.Range.getBegin();
  //   if (End.isInvalid() ||
  //       SM.isBeforeInTranslationUnit(End, Attrs.Range.getEnd()))
  //     End = Attrs.Range.getEnd();
  // }
  // DS.SetRangeStart(Begin);
  // DS.SetRangeEnd(End);
...
Decl *Parser::ParseFunctionDefinition(ParsingDeclarator &D,
  ...
  if (BodyKind != Sema::FnBodyKind::Other) {
    ...
    if (auto *funcTempDecl = llvm::dyn_cast<FunctionTemplateDecl>(Res);
        funcTempDecl != nullptr && equalSemiLoc.isValid() &&
        (BodyKind == Sema::FnBodyKind::Default ||
         BodyKind == Sema::FnBodyKind::Delete)){
      FunctionDecl *FD = funcTempDecl->getTemplatedDecl();
      if(FD)
        FD->setRangeEnd(equalSemiLoc);
    }
    // IClang end
