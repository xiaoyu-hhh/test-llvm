...
NamedDecl *Parser::ParseCXXInlineMethodDef(
  ...
  if (TryConsumeToken(tok::equal)) {
    ...
    // IClang begin
    if (auto *funcTempDecl = dyn_cast<FunctionTemplateDecl>(FnD);
        funcTempDecl != nullptr && equalSemiLoc.isValid() && Delete) {
      FunctionDecl *funcDecl = funcTempDecl->getTemplatedDecl();
      if (funcDecl)
